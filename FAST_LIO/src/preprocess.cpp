...
#if 0  // DISABLED: AVIA not used in simulation
...
#endif  // DISABLED: AVIA not used in simulation
...
void Preprocess::sim_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    pl_surf.clear();
    pl_full.clear();
    // Use PointXYZ (no intensity) since simulation clouds may not have intensity field
    ...
    for (int i = 0; i < pl_orig.points.size(); i++) {
        ...
        added_pt.y = pl_orig.points[i].y;
        added_pt.z = pl_orig.points[i].z;
        added_pt.intensity = 1.0;  // Default: no intensity in simulation cloud
        added_pt.normal_x = 0;
        added_pt.normal_y = 0;
        added_pt.normal_z = 0;
        added_pt.curvature = 0.0;
